...
| **Complaint HB resolver** | A staff member with an explicit Complaint Cases `HB_STAFF` grant. They view complaint cases and evidence and uniquely resolve Pending complaints; a Jawabu Portal HB grant does not implicitly grant Complaint Cases access. |
| **Complaint reference** | The globally unique sequential staff-facing identifier such as `CMP-1` or `CMP-1004`. Legacy references such as `CMP000001` remain valid and unchanged. Source Telegram/message IDs and UUIDs remain immutable internal identifiers and are not the operational reference shown to staff. |
...
| **Origination document packet** | The ordered generated-document set frozen for one application: exactly one officer-selected Main LAF plus any officer-selected supporting documents. Historical packets may retain older required/rule-triggered selection sources. Every document keeps its immutable template, mapping, field, signer, preview, and signing snapshot. |
| **Origination document catalogue** | The independent allowlisted library of immutable Main LAF and supporting-document versions. Documents declare eligible global products; officers choose one contract-equivalent Main LAF and optional supporting documents before application creation, and the selected versions are frozen on the application. |
| **Change of invoice name** | Inline pre-payment action on Portal invoice detail when an HB invoice carries a different national ID, commonly a spouse's. It preserves the FarmUp lead, SysUp applicant, original invoice, household link, and versioned local letter; only SysUp may change the canonical applicant identity, and payment remains blocked until a corrected replacement invoice is confirmed. It adds no Sheet fields. |
...
- `complaint_register.py` — organization-wide complaint metrics, allowlisted register/detail projections, filtered paging, and audited all-case XLSX exports
...
- `core/tests_js/tat_case_validation.test.js` — product-specific TAT amount limits and pre-submit client validation contracts
...
| `RELEASE_ALLOW_NO_BACKUP` | Explicit audited release override when the database plan cannot provide a snapshot; records `no-backup:<environment>` in release evidence, including production | No |
...
| `COMPLAINT_CATEGORY_AI_API_URL` / `COMPLAINT_CATEGORY_AI_MODEL` | Provider base URL and explicitly configured model for advisory complaint-category inference; Google AI uses the native `generateContent` contract | Treat as deployment configuration |
...
| **Complaint Cases Mini App** | `core/api/complaint_case_views.py`, `services/complaint_cases.py`, `services/complaint_register.py`, `templates/complaint_cases/`, `static/miniapp/complaint_cases.*`, `core/tests_complaint_cases.py` | Verified Telegram identity plus named group roles, a group-wide shared Pending/Resolved queue, an organization-wide allowlisted register/export, optional Sheet projection, Officer creation/reopen, Manager resolution, append-only case/evidence history, idempotent transitions, and Drive failure metadata |
...
| **Global product catalogue** | `services/product_catalog.py`, `services/product_quotes.py`, `core/models.py`, `core/admin.py`, `core/migrations/`, `core/tests_product_catalog.py` | Stable product codes, immutable effective-dated terms, Decimal money, non-overlapping publication periods, Superuser-only publication audit, explicit availability, staged unknown mappings, and version/snapshot retention on workflow cases |
...
| **TAT tracker** | `services/tat_tracker.py`, `services/tat_reporting.py`, `services/business_calendar.py`, `services/workflow_sla.py`, `services/workflow_timeline.py`, `core/api/views.py`, `templates/tat_tracker/`, `static/miniapp/tat_tracker.*`, `tat_tracker_apps_script.gs`, `core/models.py`, `core/tests_tat_tracker.py` | Role restrictions, branch/product scope, stage prerequisites, duplicate create requests, official business-hour SLA calculations, append-only event history, capability-scoped reporting, and idempotent escalation/reporting snapshots |
...
- Access-policy changes must remain maker-checker controlled and audit logged;
  the only exception is the documented Django Superuser override for direct
  `AccessGrant` administration, which still records before/after evidence and
  increments the policy version so active Mini Apps refresh their access;
