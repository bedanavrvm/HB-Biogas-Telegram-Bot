...
| **Complaint Cases Mini App** | `core/api/complaint_case_views.py`, `services/complaint_cases.py`, `templates/complaint_cases/`, `static/miniapp/complaint_cases.*`, `core/tests_complaint_cases.py` | Verified Telegram identity plus named group roles, a group-wide shared Pending/Resolved queue, Officer creation/reopen, Manager resolution, append-only case/evidence history, idempotent transitions, and Drive failure metadata |
