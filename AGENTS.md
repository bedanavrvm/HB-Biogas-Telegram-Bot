...
| **Payment batch** | A group-scoped payment workspace using only `LOAN-JAWABU` or `CASH`. Its official consecutive number is allocated on first submission, Head of Rural reviews each case, membership remains editable until an exact signed scan is accepted, and every change invalidates any superseded workbook. |
