...
| **Complaint Needs details** | A Pending complaint stored as `Review Needed` because Telegram ingestion lacked a phone number or customer ID. A Manager completes the missing identifier and confirms the category through an audited, revision-checked action before ordinary processing continues. |
