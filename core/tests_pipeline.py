...
class JblPipelineServiceTestCase(TestCase):
    ...
    @patch('core.services.sheets.GoogleSheetsService.get_instance')
    def test_second_case_appends_after_existing_row_two_on_both_tabs(self, mock_get_sheets):
        from core.tests import FakeMasterDataSheet, FakeJawabuService
        ...
        for offset, (tab, county) in enumerate((('Master Data', 'Kiambu'), ('Eco-conserve', 'Nakuru'))):
            with self.subTest(tab=tab):
                first = JawabuFarmerMaster.objects.create(
                    customer_name=f'First {tab}', national_id=f'1234567{offset * 2}',
                    primary_phone=f'25471111111{offset * 2}', county=county,
                )
                second = JawabuFarmerMaster.objects.create(
                    customer_name=f'Second {tab}', national_id=f'1234567{offset * 2 + 1}',
                    primary_phone=f'25471111111{offset * 2 + 1}', county=county,
                )
                sheet = FakeMasterDataSheet(headers)
                sheet.values = [list(headers)]
                mock_get_sheets.return_value = FakeJawabuService(sheet)

                self.assertTrue(sync_farmer_to_master_sheet(first))
                row_two = list(sheet.values[1])
                self.assertTrue(sync_farmer_to_master_sheet(second))
                self.assertEqual(mock_get_sheets.call_args.kwargs['sheet_name'], tab)
                self.assertEqual(sheet.values[1], row_two)
                self.assertEqual(sheet.values[2][headers.index('Customer Name')], f'SECOND {tab.upper()}')
                self.assertEqual(sheet.values[2][headers.index('No.')], 2)
...
class PortalMiniAppAuthTestCase(TestCase):
    ...
    def test_jbl_visit_form_uses_hybrid_numeric_date_and_native_picker(self):
        ...
        self.assertIn('Outcome <span class="required-marker" aria-hidden="true">*</span>', script)
        ...
        self.assertIn("const isJblVisit = mode === 'jbl_visit';", script)
        self.assertIn("sheetOverlay?.classList.toggle('jbl-visit-sheet', isJblVisit)", script)
        ...
        self.assertIn('class="jbl-comment-control"', script)
        self.assertIn('class="gps-capture-summary"', script)
        self.assertIn('id="btn-gps" class="secondary"><span>Capture</span>', script)
        self.assertIn('class="jbl-section-label">Visit details', script)
        self.assertIn('class="form-section form-grid jbl-details-grid"', script)
        ...
        self.assertIn('id="sheet-header-state" class="sheet-header-state"', template)
        self.assertIn('id="sheet-avatar" class="sheet-avatar"', template)
...
class JblPipelineApiTestCase(TestCase):
    ...
    def test_portal_screen_fragment_omits_shell(self):
        ...
        self.assertContains(response, 'id="page-credit"')
        self.assertNotContains(response, 'id="page-dashboard"')
    ...
    def test_portal_cold_screen_retains_shared_shell_and_only_active_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'id="portal-shell"')
        self.assertContains(response, 'id="portal-screen"')
        self.assertContains(response, 'id="page-credit"')
        self.assertContains(response, 'id="sheet-overlay"')
        self.assertNotContains(response, 'id="page-dashboard"')
        self.assertNotContains(response, 'id="page-invoices"')

    def test_each_portal_screen_cold_load_includes_shell(self):
        for screen in ('dashboard', 'jbl', 'my_visits', 'credit', 'final', 'requisition', 'deferred', 'all', 'case_history', 'batches', 'invoices', 'payments', 'history', 'imports', 'reports', 'settings'):
    ...
    def test_report_drill_down_routes_keep_the_shell_and_route_context(self):
        ...
        cases = (
            ...
            ('portal_reports_new', {}, 'edit', '', 'fields'),
            ('portal_reports_new_step', {'report_step': 'filters'}, 'edit', '', 'filters'),
            ('portal_report_screen_detail', {'report_id': report_id}, 'detail', report_id, ''),
            ('portal_report_screen_edit', {'report_id': report_id}, 'edit', report_id, 'fields'),
            ('portal_report_screen_edit_step', {'report_id': report_id, 'report_step': 'review'}, 'edit', report_id, 'review'),
            ('portal_report_screen_run', {'report_id': report_id}, 'run', report_id, ''),
        )
        for route_name, kwargs, report_view, expected_report_id, expected_step in cases:
            with self.subTest(route=route_name):
                ...
                self.assertContains(response, f'data-report-step="{expected_step}"')
                self.assertContains(response, 'data-top-level="true"' if report_view == 'catalogue' else 'data-top-level="false"')

    def test_report_editor_rejects_unknown_step(self):
        response = self.client.get('/portal/s/reports/new/not-a-report-step/')
