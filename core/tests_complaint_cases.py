...
class ComplaintCaseMiniAppAssetTests(TestCase):
    def test_compact_two_state_workspace_has_only_supported_actions(self):
        ...
        self.assertIn('data-required-capability="complaint.reports.view" hidden', template)
        self.assertIn("document.querySelectorAll('[data-required-capability]')", script)
        self.assertIn('cellPadding: 4', script)
        self.assertIn('.report-grid{--ag-cell-horizontal-padding:4px}', styles)
        ...
        self.assertIn('id="complaintGridZoomIn"', template)
        self.assertIn("storageKey: 'complaint-report-grid-zoom'", script)
