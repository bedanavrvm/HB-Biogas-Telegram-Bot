...
class ProductAdmin(CompactModelAdmin):
    ...
    def get_deleted_objects(self, objs, request):
        """Preview deletion or tombstoning without Django protected blockers."""
        ...
        for product in objs:
            ...
            tombstoned = bool(preview.blockers or legacy_statuses)
            outcome = 'will be retained as an inactive tombstone' if tombstoned else 'will be hard-deleted'
            deleted_objects.append(f'Product: {product.name} ({product.code}) — {outcome}')
            ...
            if tombstoned:
                ...
                if legacy_statuses:
                    reasons.append('governed legacy version status: ' + ', '.join(sorted(legacy_statuses)))
                deleted_objects.append('Tombstone reason: ' + '; '.join(reasons))
        return deleted_objects, model_count, set(), []

    def delete_model(self, request, obj):
        from core.services.product_catalog_full_reset import remove_product_from_catalog

        remove_product_from_catalog(
            product_id=obj.pk,
            actor=request.user,
            reason='Selected product removed through Django Admin.',
            request_id=str(uuid.uuid4()),
        )
