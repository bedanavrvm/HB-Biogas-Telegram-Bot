...
def dispatch_certificate(certificate: TatTrackerApprovalCertificate) -> dict[str, Any]:
    ...
    approval = requests.post(
        ...
        json={'telegram_user_id': certificate.signer_telegram_id},
        timeout=15,
    )
...
def certificate_payload(certificate: TatTrackerApprovalCertificate) -> dict[str, Any]:
    ...
    return {
        ...
        'context': {
            ...
            'branch_manager_name': certificate.signer_name,
        },
        'parties': [{
            'role': 'branch_manager',
            'full_name': certificate.signer_name,
            'id_number': certificate.signer_national_id,
            'phone_number': certificate.signer_phone_number,
            'email': certificate.signer_email,
            'signing_order': 1,
        }],
    }
