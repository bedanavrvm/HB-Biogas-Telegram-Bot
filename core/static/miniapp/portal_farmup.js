(() => {
  ...
  function updateSummary() {
    ...
    if (target) target.innerHTML = `<span class="selected"><strong>${c.selected}</strong><small>to commit</small></span><span class="edited"><strong>${c.editedCells}</strong><small>edits</small></span><span><strong>${c.held}</strong><small>held</small></span><span class="danger"><strong>${c.unresolved}</strong><small>unresolved</small></span>${c.hidden ? `<span><strong>${c.hidden}</strong><small>selected hidden</small></span>` : ''}`;
  ...
  function renderBatches() {
    ...
    if (badge) {
      ...
      badge.innerHTML = pending ? `${icon('calendar-days')}<span>${escapeHtml(pending.period_label || 'Current month')}: <strong>${Number(pending.remaining_count)} rows pending review</strong></span>` : '';
    }
    const action = node('farmup-pending-action');
    if (action) {
  ...
  function applicationActionLabel(value, row) {
    if (value === 'create_additional_unit') return 'Create a new unit';
    return row?._match?.kind === 'new' ? 'Create the first unit' : 'Use the existing unit';
  }
  function layoutGrid() {
  ...
  async function renderEditor() {
    ...
    if (node('farmup-pending-action')) node('farmup-pending-action').hidden = true;
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('.farmup-bulk-actions button')) event.target.closest('.farmup-bulk-actions')?.removeAttribute('open');
