(function () {
  ...
  function candidateCard(item, blocked) {
    ...
    return `<article class="payment-candidate ${blocked ? 'blocked' : ''}">
      ...
      <div class="payment-candidate-meta"><span>Invoice <strong>${escape(item.invoice_number || '-')}</strong></span><span>Order <strong>${escape(row.order_no || '-')}</strong></span><span>Balance due <strong>${escape(amount(row.hb_invoice_amount))}</strong></span><span>Branch <strong>${escape(row.branch || '-')}</strong></span></div>
  ...
  async function finalize(button) {
    ...
    if (!body || !window.confirm(`Generate final Payment #${body.payment_number} for ${body.farmer_ids.length} selected case(s)?`)) return;
    deps.setButtonLoading(button, true, 'Generating...');
    try {
      const response = await deps.portalApi.postJson('/payments/selection/', body, deps.tg, {'X-CSRFToken': deps.getCookie('csrftoken') || ''});
      if (!response.ok || !response.data?.ok) throw new Error(response.data?.error || 'Could not generate final payment.');
      deps.showToast('Final payment generated.', 'success');
      if (response.data.document?.drive_url) deps.openPortalLink(response.data.document.drive_url);
      selected.clear();
      updateSelection();
    } catch (error) {
      deps.showToast(error.message || 'Could not generate final payment.', 'error');
    } finally {
      deps.setButtonLoading(button, false);
    }
  }
