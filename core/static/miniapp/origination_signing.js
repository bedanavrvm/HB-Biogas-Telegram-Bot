(() => {
  ...
  async function api(path, options = {}) {
    const requestId = id();
    const response = await fetch(`${base}/${path}`, {credentials:'omit', cache:'no-store', ...options, headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`,'Idempotency-Key':requestId,'X-Request-ID':requestId,'X-MiniApp-Message-Contract':'2',...(options.headers||{})}});
    ...
    if (!response.ok || !data.ok) { const error = window.MiniAppUtils?.apiError ? window.MiniAppUtils.apiError(response, data) : new Error(data.message || 'We could not complete this signing step.'); error.retryAfter = response.headers.get('Retry-After'); throw error; }
    return data;
  }
  function render() {
  ...
  async function loadPage() {
    ...
    const response = await fetch(`${base}/packet/?page=${page}`, {credentials:'omit',cache:'no-store',headers:{'Authorization':`Bearer ${token}`,'X-Request-ID':id(),'X-MiniApp-Message-Contract':'2'}});
    if (!response.ok) {
      const raw = await response.json().catch(()=>({}));
      const data = window.MiniAppUtils?.normalizeResponsePayload ? window.MiniAppUtils.normalizeResponsePayload(response, raw) : raw;
  ...
  document.getElementById('save-signature').onclick=async event=>{try{event.currentTarget.disabled=true;if(reviewedPages.size<pages)throw new Error('Review every page of the loan documents before continuing.');const capture=mode==='typed'?{method:'typed',name:document.getElementById('typed-name').value.trim()}:{method:'drawn',strokes};const consent=document.getElementById('packet-consent').checked&&(session.access_mode!=='assisted'||document.getElementById('assisted-consent').checked);const data=await api('consent/',{method:'POST',body:JSON.stringify({signature_capture:capture,consent,access_mode:session.access_mode,reviewed_pages:[...reviewedPages].sort((a,b)=>a-b)})});session=data.session;render();show('Your signature is ready. Tap “Send verification code” to continue.','success');}catch(error){show(error.message,'error');}finally{event.currentTarget.disabled=false;}};
  document.getElementById('send-otp').onclick=async event=>{try{event.currentTarget.disabled=true;const data=await api('otp/',{method:'POST',body:'{}'});session=data.session;render();show(`Your six-digit verification code is being sent to ${session.phone_masked}.`,'success');document.getElementById('otp-code').focus();}catch(error){show(error.message,'error');}finally{event.currentTarget.disabled=false;}};
