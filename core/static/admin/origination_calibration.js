(function () {
  ...
  async function saveDraft() { const data = await jsonRequest(app.dataset.saveUrl, { method: 'POST', body: JSON.stringify({ revision, configuration }) }); revision = data.revision; dirty = false; return data; }
  $('calibration-save').onclick = async () => { try { status('Saving draft...'); await saveDraft(); status(`Draft revision ${revision} saved; Mini App unchanged`); } catch (error) { status(error.message, true); } };
  $('calibration-publish').onclick = async () => { try { if (dirty) { status('Saving alignment...'); await saveDraft(); } status('Publishing to Mini App...'); const data = await jsonRequest(app.dataset.publishUrl, { method: 'POST', body: JSON.stringify({ revision }) }); revision = data.revision; status(`Published revision ${revision}; regenerate the Mini App preview`); } catch (error) { status(error.message, true); } };
