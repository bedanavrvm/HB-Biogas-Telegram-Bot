"""
URL routing for the core API.
"""
from django.urls import path
from .legacy_routes import legacy_get_redirect
from .auth_views import telegram_session_login
from .miniapp_diagnostic_views import (
    miniapp_diagnostic_session_start,
    miniapp_diagnostic_signals,
)
from .views import (
    telegram_webhook,
    health_check,
    readiness_check,
    staff_telegram_activation_submit,
    jawabu_farmers_review_commit,
    miniapp_draft,
    fca_review_commit,
    order_approval_webapp_lookup,
    order_approval_webapp_suggest,
    order_approval_webapp_submit,
    spin_form_submit,
    spin_form_requests,
    spin_form_settings,
    spin_form_settings_personal,
    spin_form_complete,
    spin_form_review_update,
    spin_batch_review_resolve,
    tat_tracker_bootstrap,
    tat_tracker_home,
    tat_tracker_home_fragment,
    tat_tracker_search,
    tat_tracker_search_fragment,
    tat_tracker_target_settings,
    tat_tracker_target_sheet_sync,
    tat_tracker_settings,
    tat_tracker_settings_personal,
    tat_tracker_settings_request,
    tat_tracker_settings_review,
    tat_tracker_create,
    tat_tracker_identity_context,
    tat_tracker_task_resolve,
    tat_tracker_task_read,
    tat_tracker_tasks,
    tat_tracker_recognition,
    tat_tracker_connect_private_alerts,
    tat_tracker_disconnect_private_alerts,
    tat_tracker_detail,
    tat_tracker_update,
    tat_tracker_reports_summary,
    tat_tracker_reports_cases,
    tat_tracker_reports_export,
    tat_tracker_process_update_dispatches,
    tat_signature_webhook,
    process_messages,
    resend_unsynced,
    sync_from_sheets,
)
from credit_assessments.views import credit_assessment_action, credit_assessment_detail, credit_assessment_document
from hb_operations.views import hb_action_commissioning_notes, hb_action_correct, hb_action_detail, hb_action_document_preview, hb_action_invoice_preview, hb_action_list, hb_action_transition
from .portal_views import (
    portal_navigation,
    portal_settings,
    portal_tat_target_settings,
    portal_approval_delegations,
    portal_approval_delegation_revoke,
    portal_workspace,
    portal_workspace_views,
    portal_workspace_view_detail,
    portal_workspace_view_activate,
    portal_workspace_view_startup,
    portal_workspace_case_pin,
    portal_workspace_case_unpin,
    portal_workspace_recents_clear,
    portal_dashboard,
    portal_performance,
    portal_meta,
    portal_location_options,
    portal_imports,
    portal_import_stage,
    portal_import_detail,
    portal_import_commit,
    portal_import_archive,
    portal_import_archive_attempt,
    portal_farmup_batches,
    portal_farmup_stage,
    portal_farmup_version,
    portal_farmup_detail,
    portal_farmup_mapping,
    portal_farmup_validate,
    portal_farmup_repair_preview,
    portal_farmup_repair,
    portal_farmup_commit,
    portal_farmup_archive,
    portal_farmup_archive_attempt,
    portal_reports_catalogue,
    portal_report_preview,
    portal_curated_report,
    portal_curated_report_export,
    portal_curated_report_download,
    portal_reporting_relationships,
    portal_reports,
    portal_report_detail,
    portal_report_archive,
    portal_report_run,
    portal_report_export,
    portal_health,
    portal_set_maintenance,
    portal_jbl_queue,
    portal_jbl_queue_fragment,
    portal_my_visits,
    portal_queue_fragment,
    portal_log_jbl_visit,
    portal_jbl_visit_draft,
    portal_case_workflow_draft,
    portal_complete_jbl_visit,
    portal_create_and_complete_jbl_lead,
    portal_voice_transcription,
    portal_voice_transcription_cancel,
    portal_upload_jbl_media,
    portal_jbl_visit_completion_status,
    portal_jbl_media,
    portal_preview_jbl_media,
    portal_open_jbl_media,
    portal_open_jbl_media_signed,
    portal_open_legacy_jbl_media,
    portal_preview_case_invoice,
    portal_prepare_case_invoice_open,
    portal_open_case_invoice_signed,
    portal_clear_approval_condition,
    portal_publication_attempt,
    portal_publication_pump,
    portal_credit_queue,
    portal_set_credit_decision,
    portal_final_review_queue,
    portal_set_final_decision,
    portal_return_for_rework,
    portal_requisition_queue,
    portal_requisition_preview,
    portal_requisition_finalize,
    portal_requisition_numbering,
    portal_requisition_workbook_preview,
    portal_assign_order,
    portal_requisition_generate,
    portal_requisition_download,
    portal_requisition_batches,
    portal_requisition_batches_fragment,
    portal_requisition_batch_detail,
    portal_requisition_batch_download,
    portal_requisition_batch_retry_sync,
    portal_upload_batch_invoices,
    portal_invoice_pool,
    portal_invoice_detail,
    portal_invoice_farmer_candidates,
    portal_invoice_match,
    portal_invoice_identity_review,
    portal_invoice_name_change_create,
    portal_invoice_name_change_correct_sent,
    portal_invoice_name_change_batches,
    portal_invoice_name_change_close,
    portal_invoice_name_change_follow_up,
    portal_invoice_name_change_generate,
    portal_invoice_name_change_download,
    portal_invoice_name_change_preview,
    portal_invoice_name_change_sent,
    portal_invoice_name_change_replacement,
    portal_invoice_name_change_replacement_candidates,
    portal_invoice_unmatch,
    portal_invoice_ignore,
    portal_invoice_restore,
    portal_invoice_bulk_action,
    portal_invoice_pool_upload,
    portal_invoice_receipt_batches,
    portal_invoice_receipt_batch_detail,
    portal_invoice_receipt_batch_payment,
    portal_invoice_receipt_item_replacement,
    portal_invoice_draft_edit,
    portal_invoice_batch_confirm,
    portal_payment_readiness,
    portal_payment_candidates,
    portal_payment_selection,
    portal_payment_batches,
    portal_payment_sequence,
    portal_payment_batch_detail,
    portal_payment_batch_cases,
    portal_payment_batch_case_mode,
    portal_payment_batch_case_remove,
    portal_payment_batch_submit,
    portal_payment_batch_case_review,
    portal_payment_batch_generate,
    portal_payment_batch_cancel,
    portal_payment_batch_workbook,
    portal_payment_workbook_download,
    portal_payment_preview_data,
    portal_payment_document_preview,
    portal_payment_document_finalize,
    portal_payment_document_approve,
    portal_payment_document_regenerate,
    portal_document_history,
    portal_document_order_preview,
    portal_document_physical_signoff_upload,
    portal_document_physical_signoff_retry,
    portal_payment_document_detail,
    portal_all_cases,
    portal_deferred,
    portal_farmer_detail,
    portal_farmer_case_correction,
    portal_auth_required,
)
from .complaint_case_views import (
    complaint_cases_bootstrap,
    complaint_cases_location_options,
    complaint_cases_category_suggestion,
    complaint_cases_voice_transcription,
    complaint_cases_voice_transcription_cancel,
    complaint_cases_settings_personal,
    complaint_cases_create,
    complaint_cases_finish_created,
    complaint_cases_list,
    complaint_cases_list_fragment,
    complaint_cases_global_overview,
    complaint_cases_global_list,
    complaint_cases_global_detail,
    complaint_cases_global_export,
    complaint_cases_global_export_download,
    complaint_reports_data,
    complaint_reports_summary,
    complaint_cases_detail,
    complaint_cases_update,
    complaint_cases_complete_details,
    complaint_cases_resolve,
    complaint_cases_reopen,
    complaint_cases_evidence_access,
    complaint_cases_sync_retry,
)
from .origination_views import (
    origination_signer_session,
    origination_signer_packet_preview,
    origination_signer_consent,
    origination_signer_otp,
    origination_signer_verify,
    origination_africastalking_delivery_report,
    portal_origination_application_detail,
    portal_origination_confirm_signing,
    portal_origination_final_review,
    portal_origination_quote_preview,
    portal_origination_applications,
    portal_origination_evidence_download,
    portal_origination_evidence_remove,
    portal_origination_evidence_upload,
    portal_origination_products,
    portal_origination_prepare_review_packet,
    portal_origination_prepare_signing,
    portal_origination_recall,
    portal_origination_restart,
    portal_origination_review_packet_preview,
    portal_origination_reviewer_notice_seen,
    portal_origination_signer_session,
    portal_origination_reset_signer_session,
    portal_origination_staff_signature,
    portal_origination_production_stamp,
    portal_origination_archive_signed,
    portal_origination_signed_packet,
    portal_origination_current_signing_packet,
    portal_origination_test_signing_action,
    portal_origination_test_signing_preview,
    portal_origination_signing_requirements,
    portal_origination_preview,
    portal_origination_review,
    portal_origination_correction_takeover,
    portal_origination_submit,
    portal_origination_document_selection,
    portal_origination_document_fields,
    portal_origination_document_preview,
    portal_origination_packet_preview,
)

urlpatterns = [
    path('staff/activate/', legacy_get_redirect('staff_telegram_activation_page'), name='legacy_api_staff_telegram_activation_page'),
    path('staff/activate/submit/', staff_telegram_activation_submit, name='staff_telegram_activation_submit'),
    path(
        'miniapp-diagnostics/sessions/start/',
        miniapp_diagnostic_session_start,
        name='miniapp_diagnostic_session_start',
    ),
    path(
        'miniapp-diagnostics/sessions/<uuid:session_uuid>/signals/',
        miniapp_diagnostic_signals,
        name='miniapp_diagnostic_signals',
    ),
    path('origination/', legacy_get_redirect('loan_origination_app'), name='legacy_api_loan_origination_app'),
    path('s/', legacy_get_redirect('loan_origination_signing_short_app'), name='legacy_api_loan_origination_signing_short_app'),
    path('origination/sign/', legacy_get_redirect('loan_origination_signing_app'), name='legacy_api_loan_origination_signing_app'),
    path('origination/sign/api/session/', origination_signer_session, name='loan_origination_signer_session'),
    path('origination/sign/api/packet/', origination_signer_packet_preview, name='loan_origination_signer_packet'),
    path('origination/sign/api/consent/', origination_signer_consent, name='loan_origination_signer_consent'),
    path('origination/sign/api/otp/', origination_signer_otp, name='loan_origination_signer_otp'),
    path('origination/sign/api/verify/', origination_signer_verify, name='loan_origination_signer_verify'),
    path('origination/webhooks/africastalking/delivery/', origination_africastalking_delivery_report, name='loan_origination_africastalking_delivery'),
    path('auth/telegram/', telegram_session_login, name='telegram_session_login'),
    path('health/', health_check, name='health_check'),
    path('readiness/', readiness_check, name='readiness_check'),
    path('order-approval/', legacy_get_redirect('order_approval_form'), name='legacy_api_order_approval_form'),
    path('jawabu-farmers/review/', legacy_get_redirect('jawabu_farmers_review'), name='legacy_api_jawabu_farmers_review'),
    path('jawabu-farmers/review/commit/', jawabu_farmers_review_commit, name='jawabu_farmers_review_commit'),
    path('miniapp-drafts/<str:workflow>/<str:context_key>/', miniapp_draft, name='miniapp_draft'),
    path('origination/api/products/', portal_auth_required(portal_origination_products), name='loan_origination_products'),
    path('origination/api/applications/', portal_auth_required(portal_origination_applications), name='loan_origination_applications'),
    path('origination/api/applications/<str:application_id>/', portal_auth_required(portal_origination_application_detail), name='loan_origination_application_detail'),
    path('origination/api/applications/<str:application_id>/quote-preview/', portal_auth_required(portal_origination_quote_preview), name='loan_origination_quote_preview'),
    path('origination/api/applications/<str:application_id>/submit/', portal_auth_required(portal_origination_submit), name='loan_origination_submit'),
    path('origination/api/applications/<str:application_id>/confirm-signing/', portal_auth_required(portal_origination_confirm_signing), name='loan_origination_confirm_signing'),
    path('origination/api/applications/<str:application_id>/recall/', portal_auth_required(portal_origination_recall), name='loan_origination_recall'),
    path('origination/api/applications/<str:application_id>/restart/', portal_auth_required(portal_origination_restart), name='loan_origination_restart'),
    path('origination/api/applications/<str:application_id>/preview/', portal_auth_required(portal_origination_preview), name='loan_origination_preview'),
    path('origination/api/applications/<str:application_id>/documents/selection/', portal_auth_required(portal_origination_document_selection), name='loan_origination_document_selection'),
    path('origination/api/applications/<str:application_id>/documents/<slug:document_key>/fields/', portal_auth_required(portal_origination_document_fields), name='loan_origination_document_fields'),
    path('origination/api/applications/<str:application_id>/documents/<slug:document_key>/preview/', portal_auth_required(portal_origination_document_preview), name='loan_origination_document_preview'),
    path('origination/api/applications/<str:application_id>/packet/preview/', portal_auth_required(portal_origination_packet_preview), name='loan_origination_packet_preview'),
    path('origination/api/applications/<str:application_id>/review/', portal_auth_required(portal_origination_review), name='loan_origination_review'),
    path('origination/api/applications/<str:application_id>/final-review/', portal_auth_required(portal_origination_final_review), name='loan_origination_final_review'),
    path('origination/api/applications/<str:application_id>/correction/takeover/', portal_auth_required(portal_origination_correction_takeover), name='loan_origination_correction_takeover'),
    path('origination/api/applications/<str:application_id>/prepare-review-packet/', portal_auth_required(portal_origination_prepare_review_packet), name='loan_origination_prepare_review_packet'),
    path('origination/api/applications/<str:application_id>/review-packet/preview/', portal_auth_required(portal_origination_review_packet_preview), name='loan_origination_review_packet_preview'),
    path('origination/api/applications/<str:application_id>/prepare-signing/', portal_auth_required(portal_origination_prepare_signing), name='loan_origination_prepare_signing'),
    path('origination/api/reviewer-notices/<str:notice_id>/seen/', portal_auth_required(portal_origination_reviewer_notice_seen), name='loan_origination_reviewer_notice_seen'),
    path('origination/api/applications/<str:application_id>/signer-sessions/', portal_auth_required(portal_origination_signer_session), name='loan_origination_signer_sessions'),
    path('origination/api/applications/<str:application_id>/signer-sessions/reset/', portal_auth_required(portal_origination_reset_signer_session), name='loan_origination_reset_signer_session'),
    path('origination/api/applications/<str:application_id>/staff-signature/', portal_auth_required(portal_origination_staff_signature), name='loan_origination_staff_signature'),
    path('origination/api/applications/<str:application_id>/production-stamp/', portal_auth_required(portal_origination_production_stamp), name='loan_origination_production_stamp'),
    path('origination/api/applications/<str:application_id>/archive-signed/', portal_auth_required(portal_origination_archive_signed), name='loan_origination_archive_signed'),
    path('origination/api/applications/<str:application_id>/signed-packet/', portal_auth_required(portal_origination_signed_packet), name='loan_origination_signed_packet'),
    path('origination/api/applications/<str:application_id>/current-signing-packet/', portal_auth_required(portal_origination_current_signing_packet), name='loan_origination_current_signing_packet'),
    path('origination/api/applications/<str:application_id>/test-signing/action/', portal_auth_required(portal_origination_test_signing_action), name='loan_origination_test_signing_action'),
    path('origination/api/applications/<str:application_id>/test-signing/preview/', portal_auth_required(portal_origination_test_signing_preview), name='loan_origination_test_signing_preview'),
    path('origination/api/applications/<str:application_id>/signing-requirements/', portal_auth_required(portal_origination_signing_requirements), name='loan_origination_signing_requirements'),
    path('origination/api/applications/<str:application_id>/requirements/<slug:requirement_key>/evidence/', portal_auth_required(portal_origination_evidence_upload), name='loan_origination_evidence_upload'),
    path('origination/api/evidence/<str:evidence_id>/remove/', portal_auth_required(portal_origination_evidence_remove), name='loan_origination_evidence_remove'),
    path('origination/api/evidence/<str:evidence_id>/download/', portal_auth_required(portal_origination_evidence_download), name='loan_origination_evidence_download'),
    path('fca/review/', legacy_get_redirect('fca_review'), name='legacy_api_fca_review'),
    path('fca/review/commit/', fca_review_commit, name='fca_review_commit'),
    path('spin/', legacy_get_redirect('spin_form'), name='legacy_api_spin_form'),
    path('spin/submit/', spin_form_submit, name='spin_form_submit'),
    path('spin/requests/', spin_form_requests, name='spin_form_requests'),
    path('spin/settings/', spin_form_settings, name='spin_form_settings'),
    path('spin/settings/personal/', spin_form_settings_personal, name='spin_form_settings_personal'),
    path('spin/review/update/', spin_form_review_update, name='spin_form_review_update'),
    path('spin/batch-review/resolve/', spin_batch_review_resolve, name='spin_batch_review_resolve'),
    path('spin/complete/', spin_form_complete, name='spin_form_complete'),
    path('tat-tracker/', legacy_get_redirect('tat_tracker_app'), name='legacy_api_tat_tracker_app'),
    path('tat-tracker/bootstrap/', tat_tracker_bootstrap, name='tat_tracker_bootstrap'),
    path('tat-tracker/home/', tat_tracker_home, name='tat_tracker_home'),
    path('tat-tracker/home/fragment/', tat_tracker_home_fragment, name='tat_tracker_home_fragment'),
    path('tat-tracker/search/', tat_tracker_search, name='tat_tracker_search'),
    path('tat-tracker/search/fragment/', tat_tracker_search_fragment, name='tat_tracker_search_fragment'),
    path('tat-tracker/target-settings/', tat_tracker_target_settings, name='tat_tracker_target_settings'),
    path('tat-tracker/target-settings/sync/', tat_tracker_target_sheet_sync, name='tat_tracker_target_sheet_sync'),
    path('tat-tracker/settings/', tat_tracker_settings, name='tat_tracker_settings'),
    path('tat-tracker/settings/personal/', tat_tracker_settings_personal, name='tat_tracker_settings_personal'),
    path('tat-tracker/settings/proposals/', tat_tracker_settings_request, name='tat_tracker_settings_request'),
    path('tat-tracker/settings/proposals/review/', tat_tracker_settings_review, name='tat_tracker_settings_review'),
    path('tat-tracker/create/', tat_tracker_create, name='tat_tracker_create'),
    path('tat-tracker/identity-context/', tat_tracker_identity_context, name='tat_tracker_identity_context'),
    path('tat-tracker/tasks/resolve/', tat_tracker_task_resolve, name='tat_tracker_task_resolve'),
    path('tat-tracker/tasks/read/', tat_tracker_task_read, name='tat_tracker_task_read'),
    path('tat-tracker/tasks/', tat_tracker_tasks, name='tat_tracker_tasks'),
    path('tat-tracker/recognition/', tat_tracker_recognition, name='tat_tracker_recognition'),
    path('tat-tracker/private-alerts/connect/', tat_tracker_connect_private_alerts, name='tat_tracker_connect_private_alerts'),
    path('tat-tracker/private-alerts/disconnect/', tat_tracker_disconnect_private_alerts, name='tat_tracker_disconnect_private_alerts'),
    path('tat-tracker/detail/', tat_tracker_detail, name='tat_tracker_detail'),
    path('tat-tracker/credit-assessment/detail/', credit_assessment_detail, name='credit_assessment_detail'),
    path('tat-tracker/credit-assessment/action/', credit_assessment_action, name='credit_assessment_action'),
    path('tat-tracker/credit-assessment/document/', credit_assessment_document, name='credit_assessment_document'),
    path('tat-tracker/update/', tat_tracker_update, name='tat_tracker_update'),
    path('tat-tracker/reports/summary/', tat_tracker_reports_summary, name='tat_tracker_reports_summary'),
    path('tat-tracker/reports/cases/', tat_tracker_reports_cases, name='tat_tracker_reports_cases'),
    path('tat-tracker/reports/export/', tat_tracker_reports_export, name='tat_tracker_reports_export'),
    path('tat-tracker/update/process-dispatches/', tat_tracker_process_update_dispatches, name='tat_tracker_process_update_dispatches'),
    path('complaints/', legacy_get_redirect('complaint_cases_app'), name='legacy_api_complaint_cases_app'),
    path('complaints/bootstrap/', complaint_cases_bootstrap, name='complaint_cases_bootstrap'),
    path('complaints/location-options/', complaint_cases_location_options, name='complaint_cases_location_options'),
    path('complaints/categories/suggest/', complaint_cases_category_suggestion, name='complaint_cases_category_suggestion'),
    path('complaints/voice-transcriptions/', complaint_cases_voice_transcription, name='complaint_cases_voice_transcription'),
    path('complaints/voice-transcriptions/<uuid:attempt_id>/cancel/', complaint_cases_voice_transcription_cancel, name='complaint_cases_voice_transcription_cancel'),
    path('complaints/settings/personal/', complaint_cases_settings_personal, name='complaint_cases_settings_personal'),
    path('complaints/cases/', complaint_cases_list, name='complaint_cases_list'),
    path('complaints/cases/fragment/', complaint_cases_list_fragment, name='complaint_cases_list_fragment'),
    path('complaints/global/overview/', complaint_cases_global_overview, name='complaint_cases_global_overview'),
    path('complaints/global/cases/', complaint_cases_global_list, name='complaint_cases_global_list'),
    path('complaints/global/cases/<uuid:case_uuid>/', complaint_cases_global_detail, name='complaint_cases_global_detail'),
    path('complaints/global/export/', complaint_cases_global_export, name='complaint_cases_global_export'),
    path('complaints/global/export/download/<str:token>/', complaint_cases_global_export_download, name='complaint_cases_global_export_download'),
    path('complaints/reports/data/', complaint_reports_data, name='complaint_reports_data'),
    path('complaints/reports/summary/', complaint_reports_summary, name='complaint_reports_summary'),
    path('complaints/cases/create/', complaint_cases_create, name='complaint_cases_create'),
    path('complaints/cases/<str:case_id>/finish-created/', complaint_cases_finish_created, name='complaint_cases_finish_created'),
    path('complaints/cases/<str:case_id>/', complaint_cases_detail, name='complaint_cases_detail'),
    path('complaints/cases/<str:case_id>/update/', complaint_cases_update, name='complaint_cases_update'),
    path('complaints/cases/<str:case_id>/complete-details/', complaint_cases_complete_details, name='complaint_cases_complete_details'),
    path('complaints/cases/<str:case_id>/resolve/', complaint_cases_resolve, name='complaint_cases_resolve'),
    path('complaints/cases/<str:case_id>/reopen/', complaint_cases_reopen, name='complaint_cases_reopen'),
    path('complaints/cases/<str:case_id>/sync-retry/', complaint_cases_sync_retry, name='complaint_cases_sync_retry'),
    path('complaints/evidence/<uuid:evidence_id>/open/', complaint_cases_evidence_access, name='complaint_cases_evidence_access'),
    path('webhook/e-signatures/tat/', tat_signature_webhook, name='tat_signature_webhook'),
    path('webhook/telegram/', telegram_webhook, name='telegram_webhook'),
    path(
        'order-approval/webapp/submit/',
        order_approval_webapp_submit,
        name='order_approval_webapp_submit',
    ),
    path(
        'order-approval/webapp/lookup/',
        order_approval_webapp_lookup,
        name='order_approval_webapp_lookup',
    ),
    path(
        'order-approval/webapp/suggest/',
        order_approval_webapp_suggest,
        name='order_approval_webapp_suggest',
    ),
    path('process/messages/', process_messages, name='process_messages'),
    path('resync/unsynced/', resend_unsynced, name='resend_unsynced'),
    path('sync/from-sheets/', sync_from_sheets, name='sync_from_sheets'),

    # JBL Pipeline Portal
    path('portal/', legacy_get_redirect('portal_home'), name='legacy_api_portal_home'),
    path('portal/cases/<str:farmer_id>/', legacy_get_redirect('portal_case_history_detail'), name='legacy_api_portal_case_history_detail'),
    # Reports use drill-down routes so phone/browser Back returns through
    # report detail rather than a crowded all-in-one workspace.
    path('portal/s/reports/', legacy_get_redirect('portal_reports_screen'), name='legacy_api_portal_reports_screen'),
    path('portal/s/reports/new/', legacy_get_redirect('portal_reports_new'), name='legacy_api_portal_reports_new'),
    path('portal/s/reports/new/<str:report_step>/', legacy_get_redirect('portal_reports_new_step'), name='legacy_api_portal_reports_new_step'),
    path('portal/s/reports/<str:report_id>/', legacy_get_redirect('portal_report_screen_detail'), name='legacy_api_portal_report_screen_detail'),
    path('portal/s/reports/<str:report_id>/edit/', legacy_get_redirect('portal_report_screen_edit'), name='legacy_api_portal_report_screen_edit'),
    path('portal/s/reports/<str:report_id>/edit/<str:report_step>/', legacy_get_redirect('portal_report_screen_edit_step'), name='legacy_api_portal_report_screen_edit_step'),
    path('portal/s/reports/<str:report_id>/run/', legacy_get_redirect('portal_report_screen_run'), name='legacy_api_portal_report_screen_run'),
    # Invoices use ordinary nested routes so Telegram and phone Back move
    # through the workspace instead of reopening one crowded all-in-one view.
    path('portal/s/invoices/', legacy_get_redirect('portal_invoices_screen'), name='legacy_api_portal_invoices_screen'),
    path('portal/s/invoices/matched/', legacy_get_redirect('portal_invoices_matched'), name='legacy_api_portal_invoices_matched'),
    path('portal/s/invoices/ignored/', legacy_get_redirect('portal_invoices_ignored'), name='legacy_api_portal_invoices_ignored'),
    path('portal/s/invoices/all/', legacy_get_redirect('portal_invoices_all'), name='legacy_api_portal_invoices_all'),
    path('portal/s/invoices/name-changes/', legacy_get_redirect('portal_invoice_name_changes_screen'), name='legacy_api_portal_invoice_name_changes_screen'),
    path('portal/s/invoices/upload/', legacy_get_redirect('portal_invoices_upload'), name='legacy_api_portal_invoices_upload'),
    path('portal/s/invoices/<str:invoice_id>/', legacy_get_redirect('portal_invoice_screen_detail'), name='legacy_api_portal_invoice_screen_detail'),
    path('portal/s/<str:screen>/', legacy_get_redirect('portal_screen'), name='legacy_api_portal_screen'),
    path('portal/navigation/', portal_navigation, name='portal_navigation'),
    path('portal/settings/', portal_settings, name='portal_settings'),
    path('portal/settings/tat-targets/', portal_tat_target_settings, name='portal_tat_target_settings'),
    path('portal/settings/delegations/', portal_approval_delegations, name='portal_approval_delegations'),
    path('portal/settings/delegations/<str:delegation_id>/revoke/', portal_approval_delegation_revoke, name='portal_approval_delegation_revoke'),
    path('portal/workspace/', portal_workspace, name='portal_workspace'),
    path('portal/workspace/views/', portal_workspace_views, name='portal_workspace_views'),
    path('portal/workspace/views/<str:view_id>/', portal_workspace_view_detail, name='portal_workspace_view_detail'),
    path('portal/workspace/views/<str:view_id>/activate/', portal_workspace_view_activate, name='portal_workspace_view_activate'),
    path('portal/workspace/views/<str:view_id>/startup/', portal_workspace_view_startup, name='portal_workspace_view_startup'),
    path('portal/workspace/cases/<str:farmer_id>/pin/', portal_workspace_case_pin, name='portal_workspace_case_pin'),
    path('portal/workspace/cases/<str:farmer_id>/unpin/', portal_workspace_case_unpin, name='portal_workspace_case_unpin'),
    path('portal/workspace/recents/clear/', portal_workspace_recents_clear, name='portal_workspace_recents_clear'),
    path('portal/dashboard/', portal_auth_required(portal_dashboard), name='portal_dashboard'),
    path('portal/performance/', portal_auth_required(portal_performance), name='portal_performance'),
    path('portal/meta/', portal_auth_required(portal_meta), name='portal_meta'),
    path('portal/location-options/', portal_auth_required(portal_location_options), name='portal_location_options'),
    path('portal/imports/', portal_imports, name='portal_imports'),
    path('portal/imports/<str:kind>/stage/', portal_import_stage, name='portal_import_stage'),
    path('portal/imports/archive-attempt/', portal_import_archive_attempt, name='portal_import_archive_attempt'),
    path('portal/imports/<str:batch_id>/archive/', portal_import_archive, name='portal_import_archive'),
    path('portal/imports/<str:batch_id>/', portal_import_detail, name='portal_import_detail'),
    path('portal/imports/<str:batch_id>/commit/', portal_import_commit, name='portal_import_commit'),
    path('portal/farmup/', portal_farmup_batches, name='portal_farmup_batches'),
    path('portal/farmup/stage/', portal_farmup_stage, name='portal_farmup_stage'),
    path('portal/farmup/<str:batch_id>/versions/', portal_farmup_version, name='portal_farmup_version'),
    path('portal/farmup/archive-attempt/', portal_farmup_archive_attempt, name='portal_farmup_archive_attempt'),
    path('portal/farmup/<str:batch_id>/mapping/', portal_farmup_mapping, name='portal_farmup_mapping'),
    path('portal/farmup/<str:batch_id>/validate/', portal_farmup_validate, name='portal_farmup_validate'),
    path('portal/farmup/<str:batch_id>/repair-preview/', portal_farmup_repair_preview, name='portal_farmup_repair_preview'),
    path('portal/farmup/<str:batch_id>/repair/', portal_farmup_repair, name='portal_farmup_repair'),
    path('portal/farmup/<str:batch_id>/commit/', portal_farmup_commit, name='portal_farmup_commit'),
    path('portal/farmup/<str:batch_id>/archive/', portal_farmup_archive, name='portal_farmup_archive'),
    path('portal/farmup/<str:batch_id>/', portal_farmup_detail, name='portal_farmup_detail'),
    # IT-only, catalogue-constrained reports. These views are individually
    # Telegram-authenticated because report runs/export writes audit evidence.
    path('portal/reports/catalogue/', portal_reports_catalogue, name='portal_reports_catalogue'),
    path('portal/reports/preview/', portal_report_preview, name='portal_report_preview'),
    path('portal/reports/workspace/', portal_curated_report, name='portal_curated_report'),
    path('portal/reports/workspace/export/', portal_curated_report_export, name='portal_curated_report_export'),
    path('portal/reports/workspace/download/<str:token>/', portal_curated_report_download, name='portal_curated_report_download'),
    path('portal/reports/relationships/', portal_reporting_relationships, name='portal_reporting_relationships'),
    path('portal/reports/', portal_reports, name='portal_reports'),
    path('portal/reports/<str:report_id>/', portal_report_detail, name='portal_report_detail'),
    path('portal/reports/<str:report_id>/archive/', portal_report_archive, name='portal_report_archive'),
    path('portal/reports/<str:report_id>/run/', portal_report_run, name='portal_report_run'),
    path('portal/reports/<str:report_id>/export/', portal_report_export, name='portal_report_export'),
    path('portal/health/', portal_auth_required(portal_health), name='portal_health'),
    path('portal/health/maintenance/', portal_auth_required(portal_set_maintenance), name='portal_set_maintenance'),
    # Stage 2 - JBL Visit
    path('portal/jbl-queue/', portal_auth_required(portal_jbl_queue), name='portal_jbl_queue'),
    path('portal/jbl-queue/fragment/', portal_auth_required(portal_jbl_queue_fragment), name='portal_jbl_queue_fragment'),
    path('portal/my-visits/', portal_auth_required(portal_my_visits), name='portal_my_visits'),
    path('portal/queues/<str:queue_key>/fragment/', portal_auth_required(portal_queue_fragment), name='portal_queue_fragment'),
    path('portal/jbl-queue/create-and-complete/', portal_auth_required(portal_create_and_complete_jbl_lead), name='portal_create_and_complete_jbl_lead'),
    path('portal/jbl-queue/<str:farmer_id>/', portal_auth_required(portal_log_jbl_visit), name='portal_log_jbl_visit'),
    path('portal/jbl-queue/<str:farmer_id>/draft/', portal_auth_required(portal_jbl_visit_draft), name='portal_jbl_visit_draft'),
    path('portal/jbl-queue/<str:farmer_id>/complete-visit/', portal_auth_required(portal_complete_jbl_visit), name='portal_complete_jbl_visit'),
    path('portal/jbl-queue/<str:farmer_id>/completion-status/', portal_auth_required(portal_jbl_visit_completion_status), name='portal_jbl_visit_completion_status'),
    path('portal/farmers/<str:farmer_id>/voice-transcriptions/', portal_auth_required(portal_voice_transcription), name='portal_voice_transcription'),
    path('portal/voice-transcriptions/<str:attempt_id>/cancel/', portal_auth_required(portal_voice_transcription_cancel), name='portal_voice_transcription_cancel'),
    path('portal/publication/attempt/', portal_auth_required(portal_publication_attempt), name='portal_publication_attempt'),
    path('portal/publication/pump/', portal_auth_required(portal_publication_pump), name='portal_publication_pump'),
    path('portal/jbl-queue/<str:farmer_id>/media/', portal_auth_required(portal_upload_jbl_media), name='portal_upload_jbl_media'),
    path('portal/jbl-queue/<str:farmer_id>/media/list/', portal_auth_required(portal_jbl_media), name='portal_jbl_media'),
    path('portal/jbl-queue/<str:farmer_id>/media/<str:attachment_id>/preview/', portal_auth_required(portal_preview_jbl_media), name='portal_preview_jbl_media'),
    # Short-lived link issued by the authorised media-list API for Telegram's
    # external browser, where the Mini App initData header is unavailable.
    path('portal/jbl-media-open/<str:token>/', portal_open_jbl_media_signed, name='portal_open_jbl_media_signed'),
    path('portal/jbl-queue/<str:farmer_id>/media/legacy/<int:media_index>/open/', portal_auth_required(portal_open_legacy_jbl_media), name='portal_open_legacy_jbl_media'),
    path('portal/jbl-queue/<str:farmer_id>/media/<str:attachment_id>/open/', portal_auth_required(portal_open_jbl_media), name='portal_open_jbl_media'),
    path('portal/farmers/<str:farmer_id>/documents/invoice/<str:batch_id>/preview/', portal_auth_required(portal_preview_case_invoice), name='portal_preview_case_invoice'),
    path('portal/farmers/<str:farmer_id>/documents/invoice/<str:batch_id>/open/', portal_auth_required(portal_prepare_case_invoice_open), name='portal_prepare_case_invoice_open'),
    path('portal/case-invoice-open/<str:token>/', portal_open_case_invoice_signed, name='portal_open_case_invoice_signed'),
    path('portal/approval-conditions/<str:condition_id>/clear/', portal_auth_required(portal_clear_approval_condition), name='portal_clear_approval_condition'),
    # Stage 3 - Credit Decision
    path('portal/credit-queue/', portal_auth_required(portal_credit_queue), name='portal_credit_queue'),
    path('portal/credit-queue/<str:farmer_id>/', portal_auth_required(portal_set_credit_decision), name='portal_set_credit_decision'),
    path('portal/credit-queue/<str:farmer_id>/draft/', portal_auth_required(portal_case_workflow_draft), {'workflow': 'credit'}, name='portal_credit_decision_draft'),
    path('portal/final-review-queue/', portal_auth_required(portal_final_review_queue), name='portal_final_review_queue'),
    path('portal/final-review-queue/<str:farmer_id>/', portal_auth_required(portal_set_final_decision), name='portal_set_final_decision'),
    path('portal/final-review-queue/<str:farmer_id>/draft/', portal_auth_required(portal_case_workflow_draft), {'workflow': 'final_review'}, name='portal_final_review_draft'),
    path('portal/cases/<str:farmer_id>/return-for-rework/', portal_auth_required(portal_return_for_rework), name='portal_return_for_rework'),
    # Stage 4 - Requisition / Order (GATED)
    path('portal/requisition-queue/', portal_auth_required(portal_requisition_queue), name='portal_requisition_queue'),
    path('portal/requisition-queue/preview/', portal_auth_required(portal_requisition_preview), name='portal_requisition_preview'),
    path('portal/requisition-queue/finalize/', portal_auth_required(portal_requisition_finalize), name='portal_requisition_finalize'),
    path('portal/requisition-numbering/', portal_auth_required(portal_requisition_numbering), name='portal_requisition_numbering'),
    path('portal/requisition-queue/preview-workbook/', portal_auth_required(portal_requisition_workbook_preview), name='portal_requisition_workbook_preview'),
    path('portal/requisition-queue/generate/', portal_auth_required(portal_requisition_generate), name='portal_requisition_generate'),
    # Signed, short-lived link: Telegram opens this in the system browser,
    # where the Mini App initData header is not available.
    path('portal/requisition-download/<str:token>/', portal_requisition_download, name='portal_requisition_download'),
    path('portal/requisition-queue/<str:farmer_id>/', portal_auth_required(portal_assign_order), name='portal_assign_order'),
    path('portal/requisition-batches/', portal_auth_required(portal_requisition_batches), name='portal_requisition_batches'),
    path('portal/requisition-batches/fragment/', portal_auth_required(portal_requisition_batches_fragment), name='portal_requisition_batches_fragment'),
    path('portal/requisition-batches/upload-invoices/', portal_auth_required(portal_upload_batch_invoices), name='portal_upload_batch_invoices'),
    path('portal/requisition-batches/<str:order_number>/download/', portal_auth_required(portal_requisition_batch_download), name='portal_requisition_batch_download'),
    path('portal/requisition-batches/<str:order_number>/retry-sync/', portal_auth_required(portal_requisition_batch_retry_sync), name='portal_requisition_batch_retry_sync'),
    path('portal/requisition-batches/<str:order_number>/', portal_auth_required(portal_requisition_batch_detail), name='portal_requisition_batch_detail'),
    # Post-order HomeBiogas installation and commissioning workspace.
    path('portal/hb-actions/', hb_action_list, name='portal_hb_action_list'),
    path('portal/hb-actions/<uuid:farmer_id>/', hb_action_detail, name='portal_hb_action_api_detail'),
    path('portal/hb-actions/<uuid:farmer_id>/documents/<str:document_kind>/preview/', hb_action_document_preview, name='portal_hb_action_document_preview'),
    path('portal/hb-actions/<uuid:farmer_id>/documents/invoice/<uuid:batch_id>/preview/', hb_action_invoice_preview, name='portal_hb_action_invoice_preview'),
    path('portal/hb-actions/<uuid:farmer_id>/transition/', hb_action_transition, name='portal_hb_action_transition'),
    path('portal/hb-actions/<uuid:farmer_id>/correct/', hb_action_correct, name='portal_hb_action_correct'),
    path('portal/hb-actions/<uuid:farmer_id>/commissioning-notes/', hb_action_commissioning_notes, name='portal_hb_action_commissioning_notes'),
    path('portal/invoice-pool/', portal_auth_required(portal_invoice_pool), name='portal_invoice_pool'),
    path('portal/invoice-pool/farmers/', portal_auth_required(portal_invoice_farmer_candidates), name='portal_invoice_farmer_candidates'),
    path('portal/invoice-pool/upload/', portal_auth_required(portal_invoice_pool_upload), name='portal_invoice_pool_upload'),
    path('portal/invoice-receipts/', portal_auth_required(portal_invoice_receipt_batches), name='portal_invoice_receipt_batches'),
    path('portal/invoice-receipts/<uuid:receipt_id>/', portal_auth_required(portal_invoice_receipt_batch_detail), name='portal_invoice_receipt_batch_detail'),
    path('portal/invoice-receipts/<uuid:receipt_id>/payment/', portal_auth_required(portal_invoice_receipt_batch_payment), name='portal_invoice_receipt_batch_payment'),
    path('portal/invoice-receipt-items/<uuid:item_id>/replacement/', portal_auth_required(portal_invoice_receipt_item_replacement), name='portal_invoice_receipt_item_replacement'),
    path('portal/invoice-batches/<str:batch_id>/confirm/', portal_auth_required(portal_invoice_batch_confirm), name='portal_invoice_batch_confirm'),
    path('portal/invoice-pool/bulk-action/', portal_auth_required(portal_invoice_bulk_action), name='portal_invoice_bulk_action'),
    path('portal/invoice-pool/<str:invoice_id>/', portal_auth_required(portal_invoice_detail), name='portal_invoice_detail'),
    path('portal/invoice-pool/<str:invoice_id>/draft/', portal_auth_required(portal_invoice_draft_edit), name='portal_invoice_draft_edit'),
    path('portal/invoice-pool/<str:invoice_id>/match/', portal_auth_required(portal_invoice_match), name='portal_invoice_match'),
    path('portal/invoice-pool/<str:invoice_id>/identity-review/', portal_auth_required(portal_invoice_identity_review), name='portal_invoice_identity_review'),
    path('portal/invoice-pool/<str:invoice_id>/name-change/', portal_auth_required(portal_invoice_name_change_create), name='portal_invoice_name_change_create'),
    path('portal/invoice-name-change-items/<str:item_id>/correct-sent/', portal_auth_required(portal_invoice_name_change_correct_sent), name='portal_invoice_name_change_correct_sent'),
    path('portal/invoice-name-changes/', portal_auth_required(portal_invoice_name_change_batches), name='portal_invoice_name_change_batches'),
    path('portal/invoice-name-change-items/<str:item_id>/close/', portal_auth_required(portal_invoice_name_change_close), name='portal_invoice_name_change_close'),
    path('portal/invoice-name-change-items/<str:item_id>/follow-up/', portal_auth_required(portal_invoice_name_change_follow_up), name='portal_invoice_name_change_follow_up'),
    path('portal/invoice-name-change-items/<str:item_id>/replacement-candidates/', portal_auth_required(portal_invoice_name_change_replacement_candidates), name='portal_invoice_name_change_replacement_candidates'),
    path('portal/invoice-name-changes/<str:batch_id>/generate/', portal_auth_required(portal_invoice_name_change_generate), name='portal_invoice_name_change_generate'),
    path('portal/invoice-name-change-download/<str:token>/', portal_invoice_name_change_download, name='portal_invoice_name_change_download'),
    path('portal/invoice-name-change-preview/<str:token>/', portal_invoice_name_change_preview, name='portal_invoice_name_change_preview'),
    path('portal/invoice-name-changes/<str:batch_id>/sent/', portal_auth_required(portal_invoice_name_change_sent), name='portal_invoice_name_change_sent'),
    path('portal/invoice-name-change-items/<str:item_id>/replacement/', portal_auth_required(portal_invoice_name_change_replacement), name='portal_invoice_name_change_replacement'),
    path('portal/invoice-pool/<str:invoice_id>/unmatch/', portal_auth_required(portal_invoice_unmatch), name='portal_invoice_unmatch'),
    path('portal/invoice-pool/<str:invoice_id>/ignore/', portal_auth_required(portal_invoice_ignore), name='portal_invoice_ignore'),
    path('portal/invoice-pool/<str:invoice_id>/restore/', portal_auth_required(portal_invoice_restore), name='portal_invoice_restore'),
    path('portal/payment-documents/<str:order_number>/readiness/', portal_auth_required(portal_payment_readiness), name='portal_payment_readiness'),
    path('portal/payments/candidates/', portal_auth_required(portal_payment_candidates), name='portal_payment_candidates'),
    path('portal/payments/selection/', portal_auth_required(portal_payment_selection), name='portal_payment_selection'),
    path('portal/payments/batches/', portal_auth_required(portal_payment_batches), name='portal_payment_batches'),
    path('portal/payments/sequence/', portal_auth_required(portal_payment_sequence), name='portal_payment_sequence'),
    path('portal/payments/batches/<uuid:batch_id>/', portal_auth_required(portal_payment_batch_detail), name='portal_payment_batch_detail'),
    path('portal/payments/batches/<uuid:batch_id>/cases/', portal_auth_required(portal_payment_batch_cases), name='portal_payment_batch_cases'),
    path('portal/payments/batches/<uuid:batch_id>/cases/<uuid:farmer_id>/mode/', portal_auth_required(portal_payment_batch_case_mode), name='portal_payment_batch_case_mode'),
    path('portal/payments/batches/<uuid:batch_id>/cases/<uuid:farmer_id>/remove/', portal_auth_required(portal_payment_batch_case_remove), name='portal_payment_batch_case_remove'),
    path('portal/payments/batches/<uuid:batch_id>/submit/', portal_auth_required(portal_payment_batch_submit), name='portal_payment_batch_submit'),
    path('portal/payments/batches/<uuid:batch_id>/cases/<uuid:farmer_id>/review/', portal_auth_required(portal_payment_batch_case_review), name='portal_payment_batch_case_review'),
    path('portal/payments/batches/<uuid:batch_id>/generate/', portal_auth_required(portal_payment_batch_generate), name='portal_payment_batch_generate'),
    path('portal/payments/batches/<uuid:batch_id>/cancel/', portal_auth_required(portal_payment_batch_cancel), name='portal_payment_batch_cancel'),
    path('portal/payments/batches/<uuid:batch_id>/workbook/', portal_auth_required(portal_payment_batch_workbook), name='portal_payment_batch_workbook'),
    path('portal/payment-workbook-download/<str:token>/', portal_payment_workbook_download, name='portal_payment_workbook_download'),
    path('portal/payment-documents/<str:order_number>/preview-data/', portal_auth_required(portal_payment_preview_data), name='portal_payment_preview_data'),
    path('portal/payment-documents/<str:order_number>/preview/', portal_auth_required(portal_payment_document_preview), name='portal_payment_document_preview'),
    path('portal/payment-documents/<str:order_number>/finalize/', portal_auth_required(portal_payment_document_finalize), name='portal_payment_document_finalize'),
    path('portal/payment-document/<str:document_id>/approve/', portal_auth_required(portal_payment_document_approve), name='portal_payment_document_approve'),
    path('portal/payment-document/<str:document_id>/regenerate/', portal_auth_required(portal_payment_document_regenerate), name='portal_payment_document_regenerate'),
    path('portal/document-history/', portal_auth_required(portal_document_history), name='portal_document_history'),
    path('portal/document-history/orders/<uuid:document_id>/preview/', portal_auth_required(portal_document_order_preview), name='portal_document_order_preview'),
    path('portal/document-signoffs/<str:document_type>/<str:document_id>/upload/', portal_auth_required(portal_document_physical_signoff_upload), name='portal_document_physical_signoff_upload'),
    path('portal/document-signoffs/<str:signoff_id>/retry/', portal_auth_required(portal_document_physical_signoff_retry), name='portal_document_physical_signoff_retry'),
    path('portal/payment-document-history/<str:document_id>/', portal_auth_required(portal_payment_document_detail), name='portal_payment_document_detail'),
    # All cases + deferred
    path('portal/farmers/', portal_auth_required(portal_all_cases), name='portal_all_cases'),
    path('portal/farmers/<str:farmer_id>/', portal_auth_required(portal_farmer_detail), name='portal_farmer_detail'),
    path('portal/farmers/<str:farmer_id>/correction/', portal_auth_required(portal_farmer_case_correction), name='portal_farmer_case_correction'),
    path('portal/deferred/', portal_auth_required(portal_deferred), name='portal_deferred'),
]
