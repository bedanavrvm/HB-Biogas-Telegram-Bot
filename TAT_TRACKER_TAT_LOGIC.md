...
The key improvement needed is to make stage-level conditional formatting use stage TAT values and stage-specific targets directly.

Correct future implementation:

- Stage TAT columns should hold elapsed minutes per stage.
- Stage target values should be available in the sheet or generated by Apps Script from config.
- Conditional formatting should compare each stage's elapsed minutes to that stage's target.
- A completed stage should be green only if it met target, red if it missed target.
- A pending current stage should be amber near target and red after target.

This avoids the misleading rule where every filled stage is green even if it was completed late.
