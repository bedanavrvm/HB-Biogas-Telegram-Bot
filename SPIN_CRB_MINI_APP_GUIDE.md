...
`spin/SPIN V_2.xlsx` is the expected workbook format.

Workbook tabs:

- `Spin`: live Mini App submissions and analyst updates.
- `SPIN Legacy Batch`: `/batch` WhatsApp export imports.
- `Lists`: branch and credit analyst dropdown source values.

Django Admin configuration:

- `Sheet tab`: `Spin`
- `SPIN header row`: `2`
- `Legacy batch worksheet tab`: `SPIN Legacy Batch`

Data starts on row `3`. Row `1` is a visual group header. Row `2` is the real backend header row.

### Live `Spin` Tab Columns
...
### `SPIN Legacy Batch` Columns

Same as the live tab, but with an extra `Attachments` column before `Media URLs`:
...
`Attachments` is optional for the live `Spin` tab because `SPIN V_2.xlsx` stores uploaded file links in `Media URLs`.

Recommended dropdowns:

- `Request Type`: `SPIN/CRB`, `SPIN`, `CRB Report`
- `Customer Type`: `New`, `Existing`
- `Parse Status`: `Imported`, `Review Needed`, `Duplicate`, `Rejected`, `Failed`, `Completed`
- `Analysis Status`: `Request Received`, `Statement Requested`, `Statement Shared`, `SPIN Pending`, `SPIN Shared`, `CRB Shared`, `Analysis Shared`, `Approved`, `Rejected`, `Completed`

Column notes:

- `Request ID` is generated by Django as `SPIN-<short uuid>`.
- `Request Date/Time` is the actual request timestamp.
- `Request Month` is written by the backend as `Jul-2026` style text from `Request Date/Time` so month/year filtering works without formulas.
- `Branch` currently stores the source Telegram chat/group label unless a branch value is supplied later.
- `Credit Analyst Name` is blank until assigned or updated by analysis workflow.
- `MPESA Statement Code` is the MPESA statement access code, not a generic code field.
- `Media URLs` stores Drive links for uploaded request files.
- `Analysis Status` and `Analyst Response` are updated later by analyst completion or legacy progress detection.
## Mini App Fields
...
- SPIN header row: row containing the headers above

Optional workflow setting, generated by the admin fields above:
