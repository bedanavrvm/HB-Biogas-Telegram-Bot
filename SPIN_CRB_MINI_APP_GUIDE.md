...
- Header row: configure this in Django Admin as `header_row`; row `1` is simplest for SPIN sheets.
- Data starts: the first blank row below the header. The backend appends rows; it does not need a fixed data-start row.
- Live requests tab: use the group `Sheet tab`, normally `SPIN Requests`.
- Legacy batch tab: use `SPIN Legacy Batch` unless overridden by `legacy_batch_sheet_name`.
- Keep the same headers on both live and legacy tabs so `/spin` and `/batch` write compatible data.
...
- `Request Month` is written by the backend as a real month value from `Request Date/Time` so filters work by month/year without formulas.
- `Branch` currently stores the source Telegram chat/group label for traceability.
- `MPESA Statement Code` is the MPESA statement access code, not a generic code field.
- `Attachments` stores original uploaded filenames or WhatsApp attachment names.
