...
const CFG = {
  ...
  OWNER_EMAILS: ['bedanaurum@gmail.com', 'biogas-bot-sheets@biogas-telegram-bot.iam.gserviceaccount.com'],
  STAFF: {
    NAME:             1,
    EMAIL:            2,
    ROLE:             3,
...
function onEdit(e) {
  ...
  if ([CFG.C.NAME, CFG.C.BRANCH, CFG.C.COUNTY, CFG.C.SUB_COUNTY, CFG.C.VISITED_BY, CFG.C.HB_STAFF].includes(col))
...
function applyOrderDataValidation(sh, optionsSheet) {
  ...
  applyOptionsDropdown(sh, optionsSheet, CFG.C.SUB_COUNTY, 3);
...
function applyDecisionConditionalFormatting(sh) {
  ...
  const rules = Object.keys(CFG.COLOURS).map(decision => {
    ...
    return SpreadsheetApp.newConditionalFormatRule()
      .whenFormulaSatisfied('=$S3="' + decision + '"')
...
function buildDashboard() {
  ...
  data.forEach(row => {
    ...
    else if (dec === 'Deferred') byBranch[branch].hold++;
